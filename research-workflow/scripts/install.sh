#!/usr/bin/env bash
#
# Cross-agent installer for research-workflow.
#
...
install_cursor() {
  ...
  mkdir -p "$dir"
  local rule="$dir/research-workflow.mdc"
  if [ "$unlink" -eq 1 ]; then
...
if [ "$unlink" -eq 0 ]; then
  ...
  echo "-- 依赖体检 --"
  if [ -n "$py_bin" ]; then
    echo "  [OK]   python（必需：create_research_skeleton.py 初始化器）: $py_bin"
  else
    echo "  [缺失] python（必需）：create_research_skeleton.py 将无法运行"
