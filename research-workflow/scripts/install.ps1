# Windows native installer for research-workflow (PowerShell 5.1+).
...
function Install-Cursor {
  ...
  $body = @"
---
description: Deep-research workflow (grilling alignment + multi-agent team + red-team + review synthesis)
globs:
alwaysApply: false
---

When the user asks to deeply research / fully analyze / source-archaeology /
team-review a subsystem, module or flow in a project, read the installed
research-workflow bundle at:
$repo

First run grilling to align scope, goal, domain preset (game-mud /
software-system / none), team, output structure, adversarial mechanism,
execution mode and commit policy -- never skip grilling, never dive into source
before alignment. Then create the numbered .scratch/research/NN-topic/ skeleton
with the 6-layer dirs + brief via scripts/create_research_skeleton.py,
orchestrate the three-phase Workflow (parallel drafting -> red-team adversarial
-> review-committee synthesis), backfill any failed agents, and report.
Conclusions must be backed by first-hand repo evidence. Default: do not auto
commit.
"@
...
if (-not $Unlink) {
  ...
  Write-Output '-- 依赖体检 --'
  if ($pyBin) {
    Write-Output "  [OK]   python（必需：create_research_skeleton.py 初始化器）: $($pyBin.Source)"
  } else {
    Write-Output '  [缺失] python（必需）：create_research_skeleton.py 将无法运行'
